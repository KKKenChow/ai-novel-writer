...
- 不是简单地把所有内容都塞给AI，而是**分类构建**上下文，控制Token用量：
  1. **核心设定直取**：从已生成内容直接获取世界观(≤4000字)、人物(≤6000字)，比向量检索更完整精准
  2. **相关大纲提取**：只提取当前章节前后±2章的大纲内容(≤3000字)，避免全文塞入
  3. **前情回顾**：仅取最近2章末尾1500字作为衔接，不传全文
  4. **语义搜索补充**：用当前章节标题在向量库做语义搜索，返回最多3条语义最相关的结果；跳过已直接包含的设定/人物/大纲（避免重复），其他章节内容每条截取前800字
  5. **目标字数控制**：Prompt中硬性要求目标字数，AI输出不足70%时自动续写
  6. **max_tokens安全截断**：API层自动限制max_tokens不超过模型上限（32768），防止参数超限报错
...
A: 不会。章节生成采用**分类上下文构建**，核心设定直取（世界观≤4000字、人物≤6000字，有截断）+ 相关大纲提取（只取当前章节附近，≤3000字）+ 前章摘要（仅最近2章末尾1500字）+ 语义搜索补充（用章节标题在向量库搜索，最多3条结果，跳过已包含的设定，其他章节每条截取800字），prompt长度一直稳定，费用也稳定。如果AI一次输出不足目标字数70%，系统会自动续写（最多3轮）。
